...
top_servers_0 = df[df["Class"] == 0]["CA"].value_counts().head(5)
top_servers_1 = df[df["Class"] == 1]["Location"].value_counts().head(5)

all_top_servers = list(set(top_servers_0.index).union(set(top_servers_1.index)))

counts_0 = [df[(df["Class"] == 0) & (df["Location"] == s)].shape[0] for s in all_top_servers]
counts_1 = [df[(df["Class"] == 1) & (df["Location"] == s)].shape[0] for s in all_top_servers]
...
plt.xlabel("Lokácia")
plt.title("5 najčastejších lokácií - benígne / škodlivé")
plt.legend()
plt.tight_layout()
plt.savefig("location_class_comparison_sorted.png")
plt.show()
